defmodule TelephonyWeb.Twilio.CallController do
  ...
  def answered(conn, %{"conference_id" => conference, "conference_status_callback" => conference_status_callback}) do
    ...
    |> text(Telephony.Twiml.join_conference(conference, conference_status_callback))
  end
  ...
  def status_changed(conn, %{
    ...
    :ok = Events.publish(%Events.CallStatusChanged{
  ...
  when failed_to_connect(status) do
    :ok = Events.publish(%Events.CallFailedToJoinConference{
      conference: conference,
      providers_call_identifier: call_sid,
      call: call,
      reason: status})
